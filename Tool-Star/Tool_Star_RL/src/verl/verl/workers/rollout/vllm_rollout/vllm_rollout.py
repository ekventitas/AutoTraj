...
"""
import sys
sys.path.append('/hy-tmp/Tool-Star/Tool_Star_RL/src/verl/verl/workers/rollout/vllm_rollout/') # for importing web_search
...
class vLLMRolloutWithSearch(vLLMRollout):
    ...
    @retry(max=5, sleep=1)
    def batch_search(self, query: Union[str, List[str]], top_n=1) -> List[str]:
        if len(query) == 0:
    ...
    def search(self, query: str):
        ...
        data = {'query': query, 'top_n': 1} ### top1 for math
        response = requests.post(url, json=data)
        retrieval_text = ''
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            while active_indices:
                ...
                with self.update_sampling_params(n=1, stop=['</search>','</code>'], max_tokens=max(active_max_tokens), detokenize=True):
                ...
                for i, idx in enumerate(active_indices):
                    ...
                    if finish_reason == 'stop' and isinstance(stop_reason, str) and '</code>' in stop_reason:
